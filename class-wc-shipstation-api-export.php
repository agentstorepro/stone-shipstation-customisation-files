...
// storepro commented this code for ticket 64576
				...
				// 	// storepro customization start for measurement products
				// 	$measurement_data = $item['_measurement_data'];
				// 	if($measurement_data){
				// 		$this->xml_append( $item_xml, 'Quantity', 1, false );
				// 		$this->xml_append( $item_xml, 'UnitPrice', $item['subtotal'], false );
				// 		}
				// 		else {
...
if ( $product && $product->needs_shipping() ) {
	...
	// StorePro customization for measurement products.
	$measurement_data = $item->get_meta( '_measurement_data', true );

	if ( $measurement_data ) {
		// For measurement-based items, export as a single line item
		// with quantity 1 and total line subtotal as the unit price.
		$item_qty = 1;

		$item_total = $export_discounts_as_separate_item
...
// storepro added this code for ticket 64576
	...
	if ( ! empty( $formatted_meta ) ) {
		...
		foreach ( $formatted_meta as $meta_key => $meta ) {
			...
			// Temporary test: force Total Yardage option value to 1.
			if ( 'Total Yardage (yd)' === $display_key ) {
				$display_value = '1';
			}

			$this->xml_append( $option_xml, 'Name', $display_key );
			$this->xml_append( $option_xml, 'Value', $display_value );
			$options_xml->appendChild( $option_xml );
		}
